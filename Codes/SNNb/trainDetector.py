...
def main():
    ...
    neigh_out_classe={}
    newTrainDataFinal=None
    negativeDataset=None
    for name in range(3):
        if name != args.detectorClass:
            ...
            if type(newTrainDataFinal)==type(None):
                newTrainDataFinal=newTrainData
                negativeDataset=np.array(classDataset[name].trainData[args.fold])
            else:
                newTrainDataFinal=np.concatenate((newTrainDataFinal, newTrainData), axis=0)
                negativeDataset=np.concatenate((negativeDataset, np.array(classDataset[name].trainData[args.fold])), axis=0)
                
    
    neigh_out_classe=NearestNeighbors(n_neighbors=1, n_jobs=-1)
    neigh_out_classe.fit(newTrainDataFinal)
    ...
    for positive_index, data, negative_index in zip(neigh_in_classe.kneighbors(newTrainData)[1], classDataset[args.detectorClass].trainData[args.fold], neigh_out_classe.kneighbors(newTrainData)[1]):
        ...
        train_dataset_negative.append(negativeDataset[negative_index[0]])
